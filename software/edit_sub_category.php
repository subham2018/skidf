      ...
      <div class="content-wrapper">
        ...
        <div class="content-header row">
          <div class="content-header-left col-md-4 col-12 mb-2">
            <h3 class="content-header-title">EDIT SUB CATEGORY LIST</h3>
          </div>
...
<div class="col-md-6">
			<div class="card">
				<div class="card-header">
					<h4 class="card-title" id="basic-layout-round-controls">UPDATE SUB CATEGORY LIST</h4>
  ...
  if(isset($_POST['usubmit'])){
                    ...
                    if(!empty($_FILES['image']['tmp_name']) && is_uploaded_file( $_FILES['image']['tmp_name'] )){
                        ...
                        if($image_res != false && resize_image($image_res, $target_file, $image_type, 768, $image_width, $image_height,  $_POST['qa'], true, false)){
                            ...
                            unlink(UPLOAD_IMG_PATH . anything('sub_category','image',$_POST['bid']));
                            mysqli_query($link,"UPDATE ".$prefix."sub_category SET `image`='".$fbasename."' WHERE `id`=".$_POST['bid']);
                        }
                        else {
                            unlink($target_file);
                            $error='1';
                    ...
                    if(!isset($error) && mysqli_query($link,"UPDATE ".$prefix."sub_category SET ".$query." WHERE id=".$_POST['bid'])){
                        echo '<p class="alert alert-success"><i class="fa fa-check fa-fw"></i> Banner information successfully updated.</p>';
                        header("Location: sub_category.php?sub_id".$_REQUEST['sub_id']);
                    }
                    else { 
...
?>
						...
						<form class="form" method="post" enctype="multipart/form-data">
							<div class="form-body">
							<?php
                                $select=mysqli_query($link,"SELECT * FROM ".$prefix."sub_category WHERE `id`=".$_REQUEST['sub_id']);
            ...
            <label for="complaintinput1">Select Category Type</label>
                            <select class="form-control round" id="basicSelect" name="type" >
                             ...
                             <?php
                        $select1=mysqli_query($link,"SELECT * FROM ".$prefix."category");
                         while($row1=mysqli_fetch_object($select1)){
                         ?>
            <option value="<?=$row1->id?>" 
             <?php if ($row1->id==$row->type) { ?>
            ...
            <?=$row1->title?></option>
                  ...
                  <input type="text"  class="form-control round" placeholder="Enter Details" name="details" value=<?=$row->details?>>
                </div>       

								<div class="form-group">
									<label for="complaintinput2">Price</label>
									<input type="text" id="complaintinput2" class="form-control round" placeholder="Enter Price" name="price" value="<?=$row->price?>">
								</div>
							<div class="card-header">
                ...
                <div class="form-group">
							...
							<div class="form-actions">
								<a href="sub_category.php" class="btn btn-danger mr-1" >
