      ...
      <div class="content-wrapper">
        ...
        <div class="content-header row">
          <div class="content-header-left col-md-4 col-12 mb-2">
            <h3 class="content-header-title">Bookings for <?=anything('user','name',$_REQUEST['guest_id'])?></h3>
          </div>
	    ...
	    <div class="col-12">
	        <div class="card">
	            <div class="card-header">
	                <h4 class="card-title">Online Booking List</h4>
             ...
             $select3=mysqli_query($link,"SELECT * FROM ".$prefix."room_type WHERE `id`=".$row2->room_id);
					            ...
					            <tr>
					               ...
					               <td class="center"><?=$row3->room_type?><br>
					               	<?=$row2->room_no?></td>
												...
												<td class="center"><?=$row1->service_addon?><br> Add on Price: ₹<?=$row1->service_price?></td>
												<td class="center">₹<?=$row1->price?></td>
												<td class="center"><?=$row1->checkin?></td>
												<td class="center"><?=$row1->checkout?></td>
												<td class="center">
						...
						$select4=mysqli_query($link,"SELECT * FROM ".$prefix."onlinepayment WHERE `booking_id`=".$row1->id);
                               ...
                               if($i<$row1->price && $i!="" && $i!=$row1->price)
                               {
                               	?>
					...
					<?php } else if($i=="") { ?>
						...
						<a class="btn btn-outline-warning round btn-min-width mr-1 mb-1" href="onlineguestpayment.php?booking_id=<?=$row1->id?>&payment=<?=$row1->price?>&guest_id=<?=$_REQUEST['guest_id']?>">
							Payment</a>
							<a class="btn btn-outline-success round btn-min-width mr-1 mb-1" href="oinvoice.php?booking_id=<?=$row1->id?>&guest_id=<?=$_REQUEST['guest_id']?>">
							Invoice</a>		
