      ...
      <div class="content-wrapper">
        ...
        <div class="content-header row">
          <div class="content-header-left col-md-4 col-12 mb-2">
            <h3 class="content-header-title">EDIT ROOM LIST</h3>
          </div>
				...
				<div class="card-content collapse show">
                    ...
                    if(!empty($_FILES['side_img']['tmp_name']) && is_uploaded_file( $_FILES['side_img']['tmp_name'] )){
                        ...
                        if($image_res != false && resize_image($image_res, $target_file, $image_type, 768, $image_width, $image_height,  $_POST['qa'], true, false)){
                            ...
                            unlink(UPLOAD_IMG_PATH . anything('room_type','side_img',$_POST['bid']));
                            mysqli_query($link,"UPDATE ".$prefix."room_type SET `side_img`='".$fbasename."' WHERE `id`=".$_POST['bid']);
                        }
                        else {
                            unlink($target_file);
                            $error='1';
                    ...
                    if(!isset($error) && mysqli_query($link,"UPDATE ".$prefix."room_type SET ".$query." WHERE id=".$_POST['bid'])) //insert all text info
...
?>
						...
						<form class="form" method="post" enctype="multipart/form-data">
							<div class="form-body">
							<?php
                                $select=mysqli_query($link,"SELECT * FROM ".$prefix."room_type WHERE `id`=".$_REQUEST['property_id']);
                               $row=mysqli_fetch_object($select);
                                ?>
                   <input type="hidden" value="<?=$row->id?>" name="bid" required>  

								<div class="form-group">
									<label for="complaintinput1">Room Type Title</label>
									<input type="text" id="complaintinput1" class="form-control round" placeholder="Enter Title1" name="room_type" value="<?=$row->room_type?>">
								</div>
								<hr>
								<div class="form-body">
							...
							<div class="form-body">
								...
								<div class="form-group">
									<label for="complaintinput1">Starting Price in INR</label>
									<input type="number" id="complaintinput1" class="form-control round" placeholder="Enter Property Type" name="startprice" value="<?=$row->startprice?>">
								</div>
							</div>
              <div class="form-body">

                                <div class="form-group">
                                    <label for="complaintinput1">Maximum Adult</label>
                                    <input type="number" id="complaintinput1" class="form-control round" placeholder="Enter Maximum Adult Value" name="adult" value="<?=$row->adult?>">
                                </div>
                            </div>
                            <div class="form-body">

                                <div class="form-group">
                                    <label for="complaintinput1">Maximum Child</label>
                                    <input type="number" id="complaintinput1" class="form-control round" placeholder="Enter Maximum Child Value" name="child" value="<?=$row->child?>">
                                </div>
                            </div>
							<hr>
							<div class="card-header">
                ...
                <div class="form-group">
							...
							<div class="form-actions">
								<a href="room.php" class="btn btn-danger mr-1" >
