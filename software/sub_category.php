	    ...
	    <div class="col-12">
	        <div class="card">
	            <div class="card-header">
	                <h4 class="card-title"><?=anything('category','title',$_GET['sub_id'])?></h4>
	            ...
	            <div class="card-content collpase show">
                    ...
                    if(mysqli_query($link,"DELETE FROM ".$prefix."sub_category WHERE id=".variable_check($_GET['del']))){ //delete info
                ...
                ?>
	               ...
	               <a href="add_sub_category.php?add_sub_id=<?=$_GET['sub_id']?>"  style="width:260px;" class="btn btn-glow btn-bg-gradient-x-purple-red col-6 col-md-5 mr-1 mb-1"> Add Sub Category</a>
	                ...
	                <table class="table table-striped table-bordered sourced-data">
					        ...
					        <tbody>
							<?php
                                $select=mysqli_query($link,"SELECT * FROM ".$prefix."sub_category WHERE `type`=".$_REQUEST['sub_id']);
                               while($row=mysqli_fetch_object($select)){
                                ?>
					            <tr>
					                <td><?=$row->name?></td>
					                <td><img src="<?=UPLOAD_IMG_PATH.anything('sub_category','image',$row->id)?>" height="50px"></td>
					                <td><?=$row->price?></td>
					                
									
									<td><a class="btn btn-outline-primary round btn-min-width mr-1 mb-1"href="edit_sub_category.php?sub_id=<?=$row->id?>">Edit</a>
									
                            <a class="btn btn-outline-danger round btn-min-width mr-1 mb-1" href="?sub_id=<?=$_REQUEST['sub_id']?>&del=<?=$row->id?>">
							Delete</a></td>
					            </tr>
								<?php } ?>
					        </tbody>
