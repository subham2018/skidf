      ...
      <div class="content-wrapper">
        ...
        <div class="content-header row">
          <div class="content-header-left col-md-4 col-12 mb-2">
            <h3 class="content-header-title">Guests</h3>
          </div>
            ...
            <div class="breadcrumbs-top float-md-right">
              <div class="breadcrumb-wrapper mr-1">
                <ol class="breadcrumb">
                  ...
                  <li class="breadcrumb-item active">Manage Guest
                  </li>
                  <li class="breadcrumb-item active">Online Guest
                  </li>
                </ol>
              </div>
            </div>
	    ...
	    <div class="col-12">
	        <div class="card">
	            <div class="card-header">
	                <h4 class="card-title">Online Guest List</h4>
	            ...
	            <div class="card-content collpase show">
                    ...
                    mysqli_query($link,"UPDATE ".$prefix."guest SET `status`='".$_REQUEST['status']."' WHERE `id`=".$_REQUEST['id']);
					//update info
                    ...
                    if(mysqli_query($link,"DELETE FROM ".$prefix."guest WHERE id=".variable_check($_GET['del']))){ //deilete info
                ...
                ?>
	             ...
	             <!-- <a href="add_guest.php"  style="width:260px;" class="btn btn-glow btn-bg-gradient-x-purple-red col-6 col-md-5 mr-1 mb-1"> Add Guest</a>-->
	                ...
	                <table class="table table-striped table-bordered sourced-data">
					        ...
					        <tbody>
							<?php
                                $select=mysqli_query($link,"SELECT * FROM ".$prefix."online_booking");
                               while($row=mysqli_fetch_object($select)){
                                ?>
					            <tr>
					               <td class="center">SI/
					               	
					               	ONLINE/<?=date('Y');?>/<?=$row->guest_id?></td>
					          <td class="center"><?=anything('user','name',$row->guest_id)?></td>
												<td class="center"><?=anything('user','phone',$row->guest_id)?></td>
												<td class="center"><?=anything('user','email',$row->guest_id)?></td>
												<td class="center">Online</td>
												
												<td class="center">
													<a class="btn btn-primary round btn-min-width mr-1 mb-1" href="onlinebookings.php?guest_id=<?=$row->guest_id?>&booking_id=<?=$row->id?>">Bookings</a>
												<a class="btn btn-warning round btn-min-width mr-1 mb-1" href="kyc.php?kyc_id=<?=$row->guest_id?>">
														KYC
													</a>
													<a class="btn btn-outline-primary round btn-min-width mr-1 mb-1"href="edit_guest.php?guest_id=<?=$row->id?>">Edit</a>
									
                            <a class="btn btn-outline-danger round btn-min-width mr-1 mb-1" href="?del=<?=$row->id?>">
							Delete</a>
												
													<?php if ($row->status=='1'){ ?>
													<a class="btn btn-danger round btn-min-width mr-1 mb-1" href="?id=<?=$row->id?>&status=0">
														BLOCK
													</a>
													<?php } else {?>
													<a class="btn btn-success round btn-min-width mr-1 mb-1" href="?id=<?=$row->id?>&status=1">
														ACTIVATE
													</a>
													<?php } ?>
