	...
	<div id="invoice-template" class="card-body" >
		...
		$select=mysqli_query($link,"SELECT * FROM ".$prefix."guest WHERE `id`=".$_REQUEST['guest_id']);
                              $row=mysqli_fetch_object($select);
                                ...
                                $select1=mysqli_query($link,"SELECT * FROM ".$prefix."booking WHERE `guest_id`=".$_REQUEST['guest_id']." AND `id`=".$_REQUEST['booking_id']);
		...
		<div id="invoice-company-details" class="row">
			...
			<div class="col-md-6 col-sm-12 text-left text-md-left">							
                        ...
                        <ul class="px-0 list-unstyled">
                        	<li>To,</li>
                        	<li>GUEST ID: <?php if($row->type=="Online")
					               	{
					               		echo "ONLINE";
					               	}
					               	else {
				...
				<div class="table-responsive col-sm-12">
					<table class="table table-bordered">
					  <thead>
					    <tr>
					      <th>Booking ID</th>
					     <!--  <th class="text-left">Room Photo</th> -->
					  ...
					  <tbody>
					    <tr>
					      <th scope="row">SIB/2019/00<?=$row1->id?></th>
					      	...
					      	<!-- <th>
					      		<img src="<?=UPLOAD_IMG_PATH.$row2->side_img?>" width="150px;"></th> -->
					      		<th><p class="text-right">Room: <?=$row3->room_type?></p>
					      			<p class="text-right">Allotment: Adult- <?=$row1->adult?> Child-<?=$row1->child?></p>
					      			<p class="text-right">Room Type- <?=$row1->room_type?></p>
