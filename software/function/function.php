...
function count_rows_all($table) {
	
	global $prefix,$link;
	
	$select = mysqli_query($link,"SELECT * FROM `".$prefix.$table."`");
	return mysqli_num_rows($select);
...
function separatecol($table,$field,$sepid,$id) {
	
	global $prefix,$link;
	
	$select = mysqli_query($link,"SELECT * FROM `".$prefix.$table."` WHERE `".$sepid."`='".$id."'");
